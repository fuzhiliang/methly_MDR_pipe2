...
our ( $out_dir, $op, $seq_format, $seq_mode, $used_cycles, $sequenc_tot_cycle, $merge, $bin_size_CpG, $bin_size_nonCpG,
		$mismatch, $min_ins, $max_ins, $hit_mode, $thread, $need_trim, $discard_5_x_cycles);
...
if( $pe )	# paired-end data
{
	foreach( @all_sample )
	{
		if($need_trim == 1 )
		{
			...
			$bsaligner.= "\t$trim_PE $discard_5_x_cycles $used_cycles $read1{$_} $read2{$_} $op\_logs/$op.$_ 20 69 && touch $op\_logs/$op.$_.trim.finished\n";
...
else	# single-end data
{
	foreach( @all_sample )
	{
		if($need_trim == 1 )
		{
			...
			$bsaligner.= "\t$trim_SE $discard_5_x_cycles $used_cycles $read1{$_} $op\_logs/$op.$_ 26 69 && touch $op\_logs/$op.$_.trim.finished\n";
			$bsaligner .= "$op\_logs/$op.$_.bsalign.finished : $op\_logs/$op.$_.trim.finished\n";
